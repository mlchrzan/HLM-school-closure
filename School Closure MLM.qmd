...
closure_dist <- closure_dist |> 
  ...
  summarize(num_closed = sum(closed),
            ...
            avg_schl_1_yr_enroll_change = mean(one_yr_enroll_schl), 
            avg_schl_3_yr_enroll_change = mean(three_yr_enroll_schl), 
            avg_schl_5_yr_enroll_change = mean(five_yr_enroll_schl),
...
## Test as Non-linear Parameters

```{r fit-time}
mlfit <- glm(closed ~ as.numeric(year), 
            data = closure_CA, 
            family = 'binomial')
mqfit <- glm(closed ~ poly(as.numeric(year), 2), # quadratic effect
            data = closure_CA, 
            family = 'binomial')
mdfit <- glm(closed ~ closure_CA$year, 
            data = closure_CA, 
            family = 'binomial') # time dummies
...
-   Time should be a quadratic parameter
