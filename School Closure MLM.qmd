...
closure_dist_CA |> 
  ...
  annotate("text", 
           ...
           vjust = -0.5, hjust = 1, 
           color = 'blue4') +  # Add label for closed schools
  annotate("text", 
           x = 2003, 
           y = 8000 / 100,  
           label = "Total Schools", 
           vjust = 1, hjust = 1, 
           color = 'red4') +  # Add label for total schools
...
```{r base-model-time}
m1 <- lm(num_closed ~ year ,  data = closure_dist_CA_std)
summary(m1)
```

```{r model-random-intercept}
m2 <- lmer(num_closed ~ year + (1 | agency_id) ,  data = closure_dist_CA_std, REML = F)
summary(m2)
```
